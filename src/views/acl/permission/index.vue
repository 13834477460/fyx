...
//   children: [
//     {
//       id: "1333329957008228353", // 唯一标识
//       pid: "1", // 父级的id
//       code: "Acl", // 权限值 - 要和路由中配置的name相同
//       level: 2, // 层级 - 当前是第二级(当等级到了第四级的时候type会变成2,变成按钮权限)
//       name: "权限管理",
//       type: 1 // 类型: 1菜单 2按钮
//     },
//     {
...
// }
// 2.2 表格展示数据
// row-key="id"   ----  去渲染的数据中,找"id"这个字段,作为当前树形表格行的唯一标识
// :expand-row-keys="expandKeys"  ---  默认展开行的[唯一标识-唯一标识是row-key设置的属性]集合(数组)
...
const fetchPermissionList = async () => {
  ...
  expandKeys.value = [menuPermissionList.value[0].id as string] // ["1"]
  // expandKeys.value = ["1", "1333329957008228353"] // 做实验
